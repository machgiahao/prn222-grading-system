...
import { Award, Save, ArrowLeft, FileText, User, Eye } from "lucide-react";
...
export default function ExaminerGradingPage() {
  ...
  const [gradingDetails, setGradingDetails] = useState<GradingDetails | null>(
    null
  );
  ...
  const handleSetEmptyToZero = () => {
    ...
    const confirm = window.confirm(
      `Are you sure you want to set ${emptyCount} ungraded criteria to 0?`
    );
    if (confirm) {
      const newScores = { ...scores };
      gradingDetails.rubricItems.forEach((item) => {
        if (
          newScores[item.id] === undefined ||
          newScores[item.id] === "" ||
          newScores[item.id] === null
        ) {
          newScores[item.id] = 0;
        }
      });
  ...
  const calculateMaxScore = () => {
    ...
    return gradingDetails.rubricItems
  ...
  const handleSubmitGrade = async () => {
    ...
    if (missingScores.length > 0) {
      alert(
        `Please enter scores for all criteria. Missing: ${missingScores
          .map((i) => i.criteria)
          .join(", ")}`
      );
      return;
    ...
    if (invalidScores.length > 0) {
      alert(
        `Invalid score for: ${invalidScores
          ...
          <div className="p-8 max-w-full">
            <div className="mb-6">
              ...
              <p className="text-muted-foreground">
                Review and assign scores for each criteria
              </p>
            </div>
              ...
              <div className="rounded-lg bg-card border border-border p-6">
                ...
                <div className="flex items-center justify-between">
                  <p className="text-sm font-semibold text-foreground truncate">
                    {repoUrl ? repoUrl : "No repository URL provided"}
                  </p>

                  <Eye
                    className="w-12 h-12 text-blue-500 cursor-pointer hover:text-blue-400 transition"
              ...
              <div className="relative w-full">
                ...
                >
                  ...
                  <table
                    ...
                    style={{
                      width: `${
                        150 + gradingDetails.rubricItems.length * 180 + 100
                      }px`,
                    }}
                  >
                    <thead>
            ...
            <div className="flex justify-end gap-4">
              ...
              <Button variant="secondary" onClick={handleSetEmptyToZero}>
                Set Empty to 0
              </Button>
              <Button variant="destructive" onClick={handleSetAllZero}>
                Set All to 0
              </Button>
              <Button
